...
import { 
  ...
  Settings, Archive, Trash2, Filter, Zap, Activity
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const getCapacityStats = () => {
    ...
    // Logic: 0-10 tasks = Low, 11-25 = Optimal, 26+ = Critical
      ...
      <aside className="w-80 bg-slate-900 text-slate-300 flex flex-col fixed h-full hidden lg:flex border-r border-slate-800 shadow-2xl z-20">
        ...
        <div className="p-6 mt-4">
          <div className="bg-white/5 rounded-3xl p-5 border border-white/5 shadow-inner">
            ...
            <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
              <div 
                className={`h-full transition-all duration-1000 ${capacity.percent > 80 ? 'bg-red-500' : 'bg-blue-600'}`} 
        ...
        <div className="mt-auto p-4 border-t border-white/5">
          <div className="flex items-center gap-3 p-2 bg-white/5 rounded-2xl">
            ...
            <button onClick={() => setIsEditMode(!isEditMode)} className={`p-2 rounded-lg transition-all ${isEditMode ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>
              <Edit3 size={16}/>
            </button>
          </div>
        </div>
        ...
        <header className="flex flex-col gap-8 mb-12">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={()=>setIsModalOpen(true)} className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg hover:scale-105 transition-all"><Plus size={20}/> New Project</button>
            </div>
          </div>
            ...
            <div key={p.id} className="relative h-full group">
              ...
              {isEditMode && (
                <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm rounded-[2.5rem] z-30 flex flex-col items-center justify-center p-6 animate-in fade-in">
                  ...
                  }} className="w-20 p-3 bg-white rounded-xl text-center font-bold text-xl" />
                </div>
              )}
            </div>
          ))}
