...
import { 
  ...
  Paperclip, FileText, Download, Share2, Trash2, X, Globe, File, ChevronRight
} from 'lucide-react';
...
export default function ProjectDetail({ params }) {
  ...
  // --- NEW: ELITE PDF GENERATOR ---
  const generatePDF = () => {
    const printWindow = window.open('', '_blank');
    const content = `
      <html>
        <head>
          <title>${project.title} - Executive Summary</title>
          <style>
            body { font-family: sans-serif; padding: 40px; color: #334155; }
            h1 { color: #0f172a; margin-bottom: 5px; }
            .meta { color: #64748b; font-size: 12px; margin-bottom: 30px; text-transform: uppercase; font-weight: bold; }
            .phase { margin-top: 30px; border-top: 2px solid #f1f5f9; padding-top: 20px; }
            .phase-name { font-size: 14px; font-weight: 800; color: #1e293b; margin-bottom: 15px; }
            .task { padding: 10px 0; border-bottom: 1px solid #f8fafc; display: flex; justify-content: space-between; }
            .task-text { font-size: 13px; }
            .status { font-weight: bold; font-size: 11px; color: #3b82f6; }
          </style>
        </head>
        <body>
          <h1>${project.title}</h1>
          <div class="meta">Status: ${project.status} | Updated: ${project.lastUpdated} | Deadline: ${project.deadline}</div>
            ...
            <div class="phase">
              ...
              ${ph.tasks.map(t => `
                <div class="task">
                  ...
                  <span class="status">${t.completed ? 'COMPLETE' : 'PENDING'}</span>
                </div>
              `).join('')}
            </div>
          `).join('')}
  ...
  const handleAddResource = () => {
    ...
    const updatedPhases = project.phases.map(ph => ph.id === resourceModal.phaseId ? {
      ...ph, tasks: ph.tasks.map(t => t.id === resourceModal.taskId ? {
        ...t, resources: [...(t.resources || []), { id: Date.now(), type: resourceModal.type, value: resourceValue }]
      } : t)
    } : ph);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm p-8">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleAddResource} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold">Add</button>
            </div>
          </div>
        </div>
      )}
      ...
      <main className="flex-1 lg:ml-72 p-12 max-w-5xl mx-auto">
        <header className="flex justify-between items-start mb-12">
          ...
          <div className="flex gap-3">
            ...
            <button className="p-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 flex items-center gap-2 text-xs font-bold shadow-lg">
        ...
        <div className="space-y-12">
            ...
            <section key={phase.id}>
              <div className="flex justify-between items-center mb-5 px-2">
                <input 
                  ...
                  className="bg-transparent font-bold text-slate-400 uppercase tracking-widest text-xs outline-none focus:text-blue-600"
                />
              </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div 
                          ...
                          className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center cursor-pointer ${task.completed ? 'bg-blue-600 border-blue-600' : 'border-slate-200'}`}
                        >
                    ...
                    {task.resources?.length > 0 && (
                      <div className="mt-3 ml-10 flex flex-wrap gap-2">
                        {task.resources.map(res => (
                          <div key={res.id} className="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-bold text-slate-500 border border-slate-200 flex items-center gap-2">
                            {res.type === 'link' ? <Globe size={10}/> : <File size={10}/>} {res.value}
                          </div>
                        ))}
                      </div>
                    )}
                ...
                <div className="p-4 bg-slate-50/50">
                  {activeInput === phase.id ? (
                    <div className="flex gap-2">
                      <input autoFocus value={taskText} onChange={(e)=>setTaskText(e.target.value)} onKeyDown={(e)=>e.key==='Enter' && handleAddTask(phase.id)} className="flex-1 p-3 bg-white border rounded-xl" placeholder="Task..."/>
          ...
          <button onClick={() => {
            const updated = {...project, phases: [...project.phases, {id: Date.now(), name: "New Phase", tasks: []}]};
            save(updated);
          }} className="w-full py-6 border-2 border-dashed border-slate-200 rounded-[2.5rem] font-bold text-slate-400 hover:border-blue-300 hover:text-blue-600">+ Add Phase</button>
        </div>
      </main>
    </div>
  );
